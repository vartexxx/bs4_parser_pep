...
def pep(session):
    ...
    tags = find_tag(
        find_tag(soup, 'section', attrs={'id': 'numerical-index'}), 'tbody'
    ).find_all('tr')
    ...
    for i in tqdm(tags):
        ...
        if status_pep in statuses:
            statuses[status_pep] += 1
        if status_pep not in statuses:
            statuses[status_pep] = 1
        if status_pep not in EXPECTED_STATUS[preview_status]:
